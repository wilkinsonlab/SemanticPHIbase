...
import es.upm.cbgp.phi2ppio.logic.StaticUtils;
import es.upm.cbgp.phi2ppio.objects.NCBITaxonHierarchy;
...
public class NCBITaxonomyBioportalService {
	...
	private void saveHierarchiesMap(String f) throws Exception {
		java.util.Iterator it = this.hierarchies.keySet().iterator();
		while (it.hasNext()) {
	...
	private LinkedList<NCBITaxonHierarchy> getHierarchyForID(int id,
		...
		String cIDURL = NCBI_TAXONOMY_BIOPORTAL_URL + id + "?" + API_KEY_STRING
				+ API_KEY + "&" + FORMAT_XML;
		boolean exit = false;
		...
		String parentURLStr = NCBI_TAXONOMY_BIOPORTAL_URL + id + PARENT_STRING
